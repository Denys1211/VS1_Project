        ...
        if(taxRates && taxRates.length > 0) {
            if(templateObject.data.name) {
                ...
                let index = taxRates.findIndex(item => {
                    return item.CodeName == name;
                })
                if(index > -1) {
                    let taxRate = (taxRates[index].Rate * 100).toFixed(2) + '%';
                    var dataList = {
                        id: taxRates[index].Id || '',
                        codename: taxRates[index].CodeName || '-',
                        description: taxRates[index].Description || '-',
                        region: taxRates[index].RegionName || '-',
                        rate: taxRate || '-',
